...
const SearchBar = () => {
  ...
  // Fetch recipes when query is updated
  useEffect(() => {
    if (query) {
  ...
  // Fetch all recipes when the app initially loads (useEffect)
  useEffect(() => {
    if (!recipeData.length && !query) {
      dispatch(fetchRecipes()); // Fetch all recipes when there is no query
    }
  }, [dispatch, recipeData.length, query]);
