      ...
      {recipes?.map((recipe, index) => (
        <Col xs={24} sm={12} md={8} lg={6} key={index}>
          <Card
            ...
            cover={
              <img
                alt={recipe?.recipe?.label || 'Recipe Image'}
                src={recipe?.recipe?.image || 'default_image_url'}  // Fallback to a default image
                style={{ height: '200px', objectFit: 'cover' }}  // Ensure consistent image height
              />
            }
            title={recipe?.recipe?.label || 'Unknown Recipe'}
          >
            <p>{recipe?.recipe?.cuisineType?.join(', ') || 'No cuisine info'}</p>

            {/* Check dietLabels safely with optional chaining */}
            <p>{recipe?.recipe?.dietLabels?.length ? recipe?.recipe?.dietLabels.join(', ') : 'No dietary labels'}</p>
